...
struct PACKED log_Cust {
    ...
    uint32_t time_ms; // I
    uint8_t c_mode; // M
    // int16_t  battery_voltage; // h
    uint32_t wp_dist; // I / E
    // uint32_t wp_dist2;
    int32_t roiX; // L
    int32_t roiY; // L
    int32_t roiZ; // e
    ...
    int32_t  latitude; // L
    int32_t  longitude; // L
    int32_t  IN_altitude; // e
    // uint32_t rtl_state_log; // I
...
static const struct LogStructure log_structure[] PROGMEM = {
    ...
    { LOG_CUST_MSG, sizeof(log_Cust),       // note, gps stuff removed for ease of viewing logs for now
      "CUST", "IMILLeLLeff",      "tMS,CM,WpD,roiX,roiY,roiZ,lat,lon,inAlt,ct,DtP" },
